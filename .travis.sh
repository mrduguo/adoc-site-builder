...
DOCKER_TAG_VERSION=$TRAVIS_BRANCH-$(date -u +"%Y%m%d-%H%M%S")-$TRAVIS_COMMIT-$TRAVIS_BUILD_NUMBER
DOCKER_TAG_LATEST=$TRAVIS_BRANCH
if [ "$DOCKER_TAG_LATEST" == "master" ]; then
    DOCKER_TAG_LATEST="latest"
fi

docker build -t $TRAVIS_PULL_REQUEST_SLUG:$DOCKER_TAG_VERSION -t $TRAVIS_PULL_REQUEST_SLUGr:$DOCKER_TAG_LATEST -f Dockerfile .
